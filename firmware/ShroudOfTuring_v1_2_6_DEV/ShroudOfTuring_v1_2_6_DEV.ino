...
const unsigned long NOTE_REMOVE_HOLD_TIME = 800;  // 1s hold to remove a note from scale (v1.2.6)
            ...
            if (currentButtonState != turingLastButtonState[r][c]) {
                if (millis() - lastDebounceTime[r][c] > debounceDelayButtons) {
                    ...
                    if (currentButtonState) {
                        // Record press time for long-hold note removal (v1.2.6)
                        ...
                        // so that long-hold removal doesn't also trigger an add.
                    } else {
                        // Button released — normal mode only (no shift, no octave long-hold)
                        if (!shiftModeActive && !octaveUpLongHoldActive && !octaveDownLongHoldActive) {
                            if (noteHoldStartTime[r][c] > 0) {
                                if (millis() - noteHoldStartTime[r][c] >= NOTE_REMOVE_HOLD_TIME) {
                                    // Long-hold: remove note from scale (v1.2.6)
                                    removeNoteFromScale(noteIndex);
                                } else {
                                    // Short press: add note to scale (v1.2.6)
...
void setup() {
    ...
    #if DEBUG_MODE
        Serial.begin(9600);
        Serial.println(F("Shroud of Turing v1.2.6"));
    #endif
    for (int r = 0; r < rows; r++) {

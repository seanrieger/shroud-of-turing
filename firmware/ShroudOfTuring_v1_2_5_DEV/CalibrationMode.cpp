...
void handleCalibrationMode() {
    ...
    // Calibration mode processing - scan button matrix
    for (int c = 0; c < cols; c++) {
        ...
        for (int r = 0; r < rows; r++) {
            ...
            if (currentButtonState && !buttonState[r][c]) {
                ...
                if (octaveShift == 4 && r == 0 && c == 0) {
                    selectedNoteIndex = 48;  // Special case for High C4
                }
    ...
    // High C button serves as the "enter key" for calibration confirmation
